...
describe('PromiseProperRace', function () {
  test('throws error when number of promises is less than count argument', async function () {
    const promises = [Promise.resolve(1), Promise.resolve(2)];
    await expect(async () => await PromiseProperRace(promises, 3))
      .rejects.toThrow('Could not retrieve tx data');
  });

  test('resolves successfully when promises resolve', async function () {
    const promises = [Promise.resolve(1), Promise.resolve(2), Promise.resolve(3)];
    await expect(PromiseProperRace(promises, 3)).resolves.toEqual([1, 2, 3]);
  });

  test('resolves successfully when some promises reject', async function () {
  ...
  test('rejects when all promises reject', async function () {
